...
export const OrderEditModal: React.FC<OrderEditModalProps> = ({ order, onClose, onSaved }) => {
  ...
  const handleSave = async () => {
    if (items.length === 0) {
      setErrorMsg('La orden debe tener al menos un producto.');
      return;
    }
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 sm:p-6 bg-black/70 backdrop-blur-sm animate-in fade-in duration-200">
      ...
      >
        ...
        <div className="p-6 space-y-6">
          {errorMsg && (
            <div className="p-3 bg-[#d30005]/10 border border-[#d30005] text-[#d30005] text-xs font-semibold">
              {errorMsg}
            </div>
          )}
            ...
            <div className="border border-[#e5e5e5] divide-y divide-[#e5e5e5]">
              {items.map((item, idx) => (
                <div key={idx} className="p-3 flex items-center gap-3 text-xs">
                  ...
                  {/* Subtotal */}
                  <span className="w-20 text-right font-extrabold text-[#111111]">
          ...
          <div className="space-y-2">
            ...
            {searchResults.length > 0 && (
              <div className="border border-[#e5e5e5] divide-y divide-[#e5e5e5] max-h-48 overflow-y-auto">
                {searchResults.map((prod) => (
                  <div key={prod._id} className="p-3 text-xs">
                    <p className="font-bold text-[#111111]">{prod.name}</p>
                    <p className="text-[#707072] text-[10px]">
                      ...
                      {prod.sizesStock
                        ...
                        .map((s) => (
                          <button
                            ...
                            className="px-2 py-1 bg-[#f5f5f5] text-[#111111] font-bold border border-[#e5e5e5] hover:bg-[#111111] hover:text-white transition-all cursor-pointer"
                          >
                            Talle {s.size} ({s.stock} en stock)
                          </button>
                        ))}
                    </div>
                  </div>
          ...
          <div className="bg-[#f5f5f5] p-4 border border-[#e5e5e5] text-xs space-y-1">
            ...
            <div className="flex justify-between font-extrabold text-base text-[#111111] pt-2 border-t border-[#e5e5e5]">
              <span>Total acordado</span><span>${total.toLocaleString('es-AR')}</span>
            </div>
          </div>
        </div>
